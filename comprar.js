...
let persistentRuntimePromise = null;
...
async function crearRuntime(options = {}) {
    ...
    const trackingHandlers = {
        onRoleName: value => {
            trackingState.nombreJugador = value;
        },
        onOrderId: value => {
            trackingState.ordenId = value;
        },
        onOrderPrice: value => {
            trackingState.precioComprado = value;
        }
    };

    const page = await context.newPage();
    attachResponseTracking(page, trackingHandlers);
...
async function prepararRuntime(runtime) {
    runtime.page = await asegurarSesionActiva(runtime.context, runtime.page) || runtime.page;
    await runtime.page.waitForSelector('.wallet-entry, .region-name, .userid-login-btn, input[type="text"]', {
        state: 'visible',
        timeout: 15000
    });

    const currentRegion = await runtime.page.locator('.region-name').textContent().catch(() => '');
    if (!currentRegion.includes('Bangladesh')) {
        await navegarEnMismaPestana(runtime.page, STORE_URL, 45000);
        await runtime.page.waitForSelector('.region-name, .userid-login-btn, input[type="text"]', {
            state: 'visible',
            timeout: 15000
        });
    }
...
async function destruirRuntime(runtime) {
    if (!runtime) {
        return;
    }
...
async function ensurePersistentRuntime(options = {}) {
    if (persistentRuntime) {
        return persistentRuntime;
    }

    if (!persistentRuntimePromise) {
        persistentRuntimePromise = (async () => {
            const runtime = await crearRuntime({ ...options, keepBrowserOpen: true });
            await prepararRuntime(runtime);
            persistentRuntime = runtime;
            return runtime;
        })().finally(() => {
            persistentRuntimePromise = null;
        });
    }
...
async function resetPersistentRuntime(options = {}) {
    const previousRuntime = persistentRuntime;
    persistentRuntime = null;
    persistentRuntimePromise = null;
    await destruirRuntime(previousRuntime);
    return ensurePersistentRuntime(options);
}

async function cerrarPersistentRuntime() {
    const previousRuntime = persistentRuntime;
    persistentRuntime = null;
    persistentRuntimePromise = null;
    await destruirRuntime(previousRuntime);
...
function obtenerEstadoPersistentRuntime() {
    return {
        ready: Boolean(persistentRuntime),
        warmingUp: Boolean(persistentRuntimePromise),
        createdAt: persistentRuntime?.createdAt || null,
        lastReadyAt: persistentRuntime?.lastReadyAt || null,
        headless: persistentRuntime?.headless ?? RUN_HEADLESS
    };
}

async function ejecutarCompraConRuntime(runtime, executionConfig) {
    let paqueteComprado = '';
    runtime.trackingState.nombreJugador = '';
    runtime.trackingState.ordenId = '';
    runtime.trackingState.precioComprado = '';

    runtime.page = await prepararRuntime(runtime);

    console.log('🎮 Ingresando ID de personaje:', executionConfig.targetRoleId);
    await ingresarRoleIdYEsperarProductos(runtime.page, executionConfig.targetRoleId);

    if (executionConfig.mapPackages) {
        await runtime.page.waitForSelector('.goods-item, [data-goodsid], [data-goods-id]', {
            state: 'visible',
            timeout: 15000
        });
        const paquetes = await extraerPaquetesVisibles(runtime.page);
    ...
    await runtime.page.waitForSelector('.goods-item, [data-goodsid], [data-goods-id]', { state: 'visible', timeout: 10000 });
    if (executionConfig.packageKey) {
        paqueteComprado = await seleccionarPaqueteMapeado(runtime.page, executionConfig.packageKey);
    } else {
        paqueteComprado = await seleccionarPaquete(runtime.page, CONFIG.goldPackage);
    }
    console.log(`✅ Paquete seleccionado: ${paqueteComprado}`);

    if (await carritoYaExpuesto(runtime.page)) {
        console.log('🛒 Carrito ya expuesto en pantalla');
    } else {
        console.log('🛒 Abriendo carrito...');
        const carrito = runtime.page.locator('#sidebarCart, .sidebar-cart').first();
        if (await carrito.isVisible().catch(() => false)) {
            await carrito.click();
        } else {
            await runtime.page.locator('.shopping-cart-entry').first().click().catch(() => {});
        }
    }
    await runtime.page.waitForSelector('.shopping-with-gift-cards .bui-checkbox, .gc-checkbox-mobile', { state: 'visible', timeout: 10000 });

    console.log('💳 Activando NetEase Pay...');
    await activarNetEasePay(runtime.page);

    console.log('🛒 Procesando Top-up...');
    const topUpBtn = await obtenerBotonTopUp(runtime.context, runtime.page);
    const firstInput = await clickTopUpAndEsperarPin(runtime.page, topUpBtn);

    console.log('🔐 Ingresando PIN...');
    await firstInput.click();
    for (const pinDigit of executionConfig.pin.split('')) {
        await runtime.page.keyboard.press(pinDigit, { delay: 50 });
    }

    const okBtn = runtime.page.locator('.action-btn.bui-button-primary, button:has-text("OK")').first();
    ...
    return construirResultadoCompra({
        success: true,
        roleId: executionConfig.targetRoleId,
        jugador: runtime.trackingState.nombreJugador,
        paquete: paqueteComprado || CONFIG.goldPackage.goodsinfo,
        precio: runtime.trackingState.precioComprado,
        orden: runtime.trackingState.ordenId,
        packageKey: executionConfig.packageKey || null
    });
}

async function ejecutarCompraPersistente(options = {}) {
    const executionConfig = construirExecutionConfig({ ...options, keepBrowserOpen: true });
    validarConfiguracion(executionConfig);
    const tiempoInicio = Date.now();

    for (let intento = 1; intento <= 2; intento++) {
        try {
            const runtime = intento === 1
                ? await ensurePersistentRuntime(executionConfig)
                : await resetPersistentRuntime(executionConfig);
            const resultado = await ejecutarCompraConRuntime(runtime, executionConfig);
            const tiempoTotal = ((Date.now() - tiempoInicio) / 1000).toFixed(2);
            return {
                ...resultado,
                tiempo: `${tiempoTotal} segundos`,
                mode: resultado.mode || 'buy-package',
...
async function ejecutarCompra(options = {}) {
    ...
    if (options.persistentSession) {
        return ejecutarCompraPersistente(options);
    }
